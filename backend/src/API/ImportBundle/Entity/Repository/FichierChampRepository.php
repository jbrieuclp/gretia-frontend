...
class FichierChampRepository extends EntityRepository
{
    ...
    **/
    public function replaceElement($champ, $search, $replace)
    {
        ...
        $requete->bindValue(':search', $oldValue);
    ...
    public function getListObservers($field)
    {
        ...
        $sql = "
            WITH observer (observer) as (
                SELECT DISTINCT trim(unnest(string_to_array(".$field->getChamp().", '|')))
                FROM ".$field->getFichier()->getTable()."
            )
            ...
            SELECT 
                ...
                COALESCE(jsonb_agg(DISTINCT CONCAT_WS(' ', NULLIF(nom.nom_role, ''), NULLIF(nom.prenom_role, ''))) FILTER ( WHERE CONCAT_WS(' ', NULLIF(nom.nom_role, ''), NULLIF(nom.prenom_role, '')) <> ''), '[]'::jsonb) as proposition
            FROM observer
            ...
            WHERE presence.id_role IS NULL
