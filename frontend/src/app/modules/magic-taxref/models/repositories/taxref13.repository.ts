...
@Injectable()
export class Taxref13Repository {
  ...
  getTaxon(cd_nom: number): Observable<Taxref13> {
  	const taxonUrl = this.httpUrlBase + '/taxref/12/taxons/' + cd_nom;
  	const options = {};
    return this.http
    	.get(taxonUrl, options)
    	.pipe(
    		map((response: object): Taxref13 => { 
	    		return new Taxref13().deserialize(response);
	    	})
	   	);
  }

  /** GET taxon par ID (cd_ref) **/
  getSynonymes(cd_ref: number): Observable<Taxref13[]> {
  	const taxonUrl = this.httpUrlBase + '/taxref/12/taxons/' + cd_ref + '/synonymes';
  	const options = {};
    return this.http
    	.get(taxonUrl, options)
    	.pipe(
    		map((response: Array<object>): Taxref13[] => { 
	    		return response.map((json: object) => new Taxref13().deserialize(json));
	    	})
	    );
  }
  ...
  getTaxonFromTaxrefV(version:number, cd_nom:number) {
    ...
    return this.http.get<Taxref13>(`http://taxref-match.loc/app_dev.php/api/taxref${version}/${cd_nom}`, {params: params})
                                    ;
  }

  /** GET taxon par ID (cd_ref) **/
  getParents(cd_ref: number): Observable<Taxref13[]> {
  ...
  getChildrens(cd_ref: number): Observable<Taxref13[]> {
